...
if parametersets == ['autotuneNP']:
    index_key_name = index_key +"_NP"
...
if not os.path.exists(tmp_dir):
    print "%s does not exist, creating it" % tmp_dir
    os.mkdir(tmp_dir)
    
if not os.path.exists(db_dir+"/bigann"):
    os.mkdir(db_dir+"/bigann")
    os.mkdir(db_dir+"/bigann/gnd")
    
if not os.path.exists(db_dir+"/deep1b"):
    os.mkdir(db_dir+"/deep1b")
...
if dbname.startswith('SIFT'):
    ...
    xq = mmap_bvecs(db_dir+'/bigann/bigann_query.bvecs')
    gt = ivecs_read(db_dir+'/bigann/gnd/idx_%dM.ivecs' % dbsize)

elif dbname == 'Deep1B':
    
    xq = mmap_fvecs(db_dir+'/deep1b/deep1B_queries.fvecs')
    gt = ivecs_read(db_dir+'/deep1b/deep1B_groundtruth.ivecs')
...
def get_trained_index():
    ...
    if not os.path.exists(filename):
        ...
        if dbname.startswith('SIFT'):
            # SIFT1M to SIFT1000M
            dbsize = int(dbname[4:-1])
            xt = mmap_bvecs(db_dir+'/bigann/bigann_learn.bvecs')
        
        elif dbname == 'Deep1B':
            xt = mmap_fvecs(db_dir+'/deep1b/learn.fvecs')
            xt = xt[:10 * 1000 * 1000]
...
def get_populated_index():
    ...
    if not os.path.exists(filename):
        ...
        #Open main dataset only if prepopulated index is not available.
        if dbname.startswith('SIFT'):
            # SIFT1M to SIFT1000M
            dbsize = int(dbname[4:-1])
            xb = mmap_bvecs(db_dir+'/bigann/bigann_base.bvecs')
            # trim xb to correct size
            xb = xb[:dbsize * 1000 * 1000]
        elif dbname == 'Deep1B':
            xb = mmap_fvecs(db_dir+'/deep1b/base.fvecs')
        else:
            print >> sys.stderr, 'unknown dataset', dbname
            sys.exit(1)
        ...
        i0 = 0
        t0 = time.time()
        for xs in matrix_slice_iterator(xb, 1000000):
            i1 = i0 + xs.shape[0]
            ...
            sys.stdout.flush()
            i0 = i1
        #index.add(xb) # Altered to benefit from IVFVPQ parallelisation
        print
...
if parametersets == ['autotune'] or parametersets == ['autotuneMT'] or parametersets == ['autotuneNP'] :

    if parametersets == ['autotune'] or parametersets == ['autotuneNP'] :
        faiss.omp_set_num_threads(1)

    if parametersets == ['autotuneNP']:
        ps.restrict_range('ht',0)
    ...
    t0 = time.time()
    op = ps.explore_limit_time(index, xq, crit, 2)
    print "Done in %.3f s, available OPs:" % (time.time() - t0)

    # opv is a C++ vector, so it cannot be accessed like a Python array
    opv = op.optimal_pts
    
    with open("/tmp/%s_%s-r1.txt"%(dbname, index_key_name),"w+") as f:
        print >> f,  "%-40s  R1   time" % "Parameters"
        for i in range(opv.size()):
            opt = opv.at(i)
            print >> f, "%-40s  %.4f  %7.3f" % (opt.key, opt.perf, opt.t)
            f.flush()
    ...
    # setup the Criterion object: optimize for 1-R@100
    crit = faiss.OneRecallAtRCriterion(nq, 100)
    # by default, the criterion will request only 1 NN
    crit.nnn = 100
    crit.set_groundtruth(None, gt.astype('int64'))
    ...
    # opv is a C++ vector, so it cannot be accessed like a Python array
    opv = op.optimal_pts
    with open("/tmp/%s_%s-r100.txt"%(dbname, index_key_name),"w+") as f:
        print >> f,  "%-40s  R100   time" % "Parameters"
        for i in range(opv.size()):
            opt = opv.at(i)
            print >> f, "%-40s  %.4f  %7.3f" % (opt.key, opt.perf, opt.t)
        f.flush()
    ...
    for param in parametersets:
        ...
        remaining_expe-=1
        while(remaining_expe > 0):
            Di, Ii = index.search(xq, 100)
            remaining_expe-=1
        t1 = time.time()
        ...
        print "%8.3f  " % ((t1 - t0) * 1000.0 / nq / repeat_expe),
